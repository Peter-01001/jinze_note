我来实现一个命令行版本的2048游戏，使用Python语言。

## 1. 程序的设计思想

### 整体架构：
- 使用面向对象设计，将游戏逻辑封装在 `Game2048` 类中
- 核心数据结构：4×4的二维数组表示游戏棋盘
- 游戏流程：初始化→显示棋盘→获取输入→移动处理→生成新数字→判断游戏状态

### 关键算法：
1. **移动合并算法**：将一行/列的数字进行合并（相同数字合并，空位补0）
2. **方向转换**：通过矩阵转置和反转来统一处理四个方向的移动
3. **随机数生成**：在空白位置随机生成2或4（2的概率为90%，4的概率为10%）
4. **游戏状态判断**：检查是否胜利（出现2048）、失败（无空位且无法合并）、继续

## 2. 核心代码说明

```python
def move(self, direction):
    """核心移动函数，处理四个方向的移动"""
    moved = False
    
    # 统一处理：都转换为向左移动的逻辑
    for i in range(4):
        # 根据方向提取一行/一列
        if direction == 'w':  # 上
            line = [self.board[j][i] for j in range(4)]
        elif direction == 's':  # 下
            line = [self.board[3-j][i] for j in range(4)]
        elif direction == 'a':  # 左
            line = self.board[i][:]
        else:  # 右
            line = self.board[i][::-1]
        
        # 合并相同的数字
        new_line = self.merge(line)
        
        # 将合并后的行/列放回棋盘
        if direction == 'w':
            for j in range(4):
                if self.board[j][i] != new_line[j]:
                    moved = True
                self.board[j][i] = new_line[j]
        elif direction == 's':
            for j in range(4):
                if self.board[3-j][i] != new_line[j]:
                    moved = True
                self.board[3-j][i] = new_line[j]
        elif direction == 'a':
            if self.board[i] != new_line:
                moved = True
            self.board[i] = new_line
        else:  # 右
            if self.board[i][::-1] != new_line:
                moved = True
            self.board[i] = new_line[::-1]
    
    return moved
```

**merge函数详解**：
```python
def merge(self, line):
    """合并一行的数字，如 [2, 2, 4, 0] -> [4, 4, 0, 0]"""
    # 1. 移除0，将数字向左靠拢
    new_line = [num for num in line if num != 0]
    
    # 2. 合并相邻的相同数字
    i = 0
    while i < len(new_line) - 1:
        if new_line[i] == new_line[i + 1]:
            new_line[i] *= 2
            new_line.pop(i + 1)
        i += 1
    
    # 3. 补0到长度为4
    new_line += [0] * (4 - len(new_line))
    return new_line
```

## 3. 完整程序代码

```python
import random
import os

class Game2048:
    def __init__(self):
        """初始化游戏"""
        self.board = [[0] * 4 for _ in range(4)]
        self.score = 0
        self.game_over = False
        self.win = False
        self.init_game()
    
    def init_game(self):
        """游戏初始化，生成两个初始数字"""
        for _ in range(2):
            self.generate_new_number()
    
    def generate_new_number(self):
        """在空白位置随机生成2或4"""
        empty_cells = []
        
        # 找出所有空白位置
        for i in range(4):
            for j in range(4):
                if self.board[i][j] == 0:
                    empty_cells.append((i, j))
        
        if empty_cells:
            i, j = random.choice(empty_cells)
            # 90%概率生成2，10%概率生成4
            self.board[i][j] = 2 if random.random() < 0.9 else 4
    
    def merge(self, line):
        """合并一行的数字"""
        # 移除0，将数字向左靠拢
        new_line = [num for num in line if num != 0]
        
        # 合并相邻的相同数字
        i = 0
        while i < len(new_line) - 1:
            if new_line[i] == new_line[i + 1]:
                new_line[i] *= 2
                self.score += new_line[i]  # 更新分数
                new_line.pop(i + 1)
            i += 1
        
        # 补0到长度为4
        new_line += [0] * (4 - len(new_line))
        return new_line
    
    def move(self, direction):
        """处理移动"""
        moved = False
        
        # 统一处理：都转换为向左移动的逻辑
        for i in range(4):
            # 根据方向提取一行/一列
            if direction == 'w':  # 上
                line = [self.board[j][i] for j in range(4)]
            elif direction == 's':  # 下
                line = [self.board[3-j][i] for j in range(4)]
            elif direction == 'a':  # 左
                line = self.board[i][:]
            else:  # 右
                line = self.board[i][::-1]
            
            # 合并相同的数字
            new_line = self.merge(line)
            
            # 将合并后的行/列放回棋盘
            if direction == 'w':
                for j in range(4):
                    if self.board[j][i] != new_line[j]:
                        moved = True
                    self.board[j][i] = new_line[j]
            elif direction == 's':
                for j in range(4):
                    if self.board[3-j][i] != new_line[j]:
                        moved = True
                    self.board[3-j][i] = new_line[j]
            elif direction == 'a':
                if self.board[i] != new_line:
                    moved = True
                self.board[i] = new_line
            else:  # 右
                if self.board[i][::-1] != new_line:
                    moved = True
                self.board[i] = new_line[::-1]
        
        return moved
    
    def check_game_status(self):
        """检查游戏状态"""
        # 检查是否达到2048
        for row in self.board:
            if 2048 in row:
                self.win = True
                return "win"
        
        # 检查是否还有空位
        for row in self.board:
            if 0 in row:
                return "continue"
        
        # 检查是否还能合并
        for i in range(4):
            for j in range(4):
                # 检查右侧是否可以合并
                if j < 3 and self.board[i][j] == self.board[i][j + 1]:
                    return "continue"
                # 检查下侧是否可以合并
                if i < 3 and self.board[i][j] == self.board[i + 1][j]:
                    return "continue"
        
        self.game_over = True
        return "game over"
    
    def display_board(self):
        """显示游戏棋盘"""
        os.system('cls' if os.name == 'nt' else 'clear')  # 清屏
        
        print("=" * 35)
        print("          2048 GAME")
        print("=" * 35)
        print(f"Score: {self.score}")
        print()
        
        for row in self.board:
            print("+------+------+------+------+")
            print("|", end="")
            for cell in row:
                if cell == 0:
                    print("      |", end="")
                else:
                    print(f"{cell:^6}|", end="")
            print()
        print("+------+------+------+------+")
        print()
        
        if self.win:
            print("🎉 Congratulations! You reached 2048!")
            print("You can continue playing...")
        elif self.game_over:
            print("💔 Game Over! No more moves available.")
        else:
            print("Controls: w(↑) a(←) s(↓) d(→) q(quit)")
    
    def run(self):
        """运行游戏主循环"""
        print("Welcome to 2048!")
        print("Use WASD keys to move the tiles.")
        print("Try to reach 2048!")
        print()
        
        while True:
            self.display_board()
            
            if self.game_over or self.win:
                choice = input("Play again? (y/n): ").lower()
                if choice == 'y':
                    self.__init__()
                    continue
                else:
                    break
            
            # 获取玩家输入
            move_map = {
                'w': 'up',
                'a': 'left',
                's': 'down',
                'd': 'right',
                'q': 'quit'
            }
            
            direction = input("Enter move (w/a/s/d) or q to quit: ").lower()
            
            if direction == 'q':
                print("Thanks for playing!")
                break
            
            if direction not in ['w', 'a', 's', 'd']:
                print("Invalid input! Use w/a/s/d")
                input("Press Enter to continue...")
                continue
            
            # 执行移动
            moved = self.move(direction)
            
            if moved:
                # 生成新数字
                self.generate_new_number()
                # 检查游戏状态
                self.check_game_status()


def main():
    """主函数"""
    game = Game2048()
    game.run()


if __name__ == "__main__":
    main()
```

## 4. 运行结果截图

由于这里是文本环境，我无法提供真实的截图，但我可以展示游戏运行时的典型界面：

```
===================================
          2048 GAME
===================================
Score: 28

+------+------+------+------+
|      |      |      |      |
+------+------+------+------+
|   2  |      |      |      |
+------+------+------+------+
|      |   4  |   2  |      |
+------+------+------+------+
|      |      |      |   4  |
+------+------+------+------+

Controls: w(↑) a(←) s(↓) d(→) q(quit)
Enter move (w/a/s/d) or q to quit:
```

### 游戏特性说明：

1. **清晰的界面**：使用ASCII字符绘制4×4的棋盘
2. **分数系统**：实时显示当前得分
3. **方向控制**：
   - w：向上移动
   - a：向左移动
   - s：向下移动
   - d：向右移动
   - q：退出游戏
4. **游戏状态提示**：
   - 达到2048时显示祝贺信息
   - 游戏结束时提示
   - 可以继续游戏或重新开始
5. **跨平台支持**：自动识别操作系统进行清屏

### 游戏流程示例：
1. 游戏开始时有两个随机位置的数字（2或4）
2. 玩家输入移动方向
3. 数字向该方向移动并合并相同数字
4. 在空白位置生成新数字（2或4）
5. 检查是否达到2048或游戏结束
6. 重复2-5步直到游戏结束或玩家退出

这个实现包含了2048游戏的所有核心功能，代码结构清晰，易于理解和扩展。你可以通过添加颜色、保存最高分等功能来进一步丰富游戏体验。