...
**数据库系统组成**：由数据库、数据库管理系统、应用程序和数据库管理员组成的系统
...
拒绝、级联操作、设置空值
    ...
    ON DELETE CASCADE
    /*当删除Student表中的元组时，级联删除SC 表中相应的元组*/
    ON UPDATE CASCADE,
    /*当更新Student表中的sno 时，级联更新SC 表中相应的元组*/
    FOREIGN KEY(Cno)REFERENCES Course(Cno)   /*在表级定义参照完整性*/
    ON DELETE NO ACTION
    /*当删除Course 表中的元组造成与SC 表不一致时，拒绝删除*/
    ON UPDATE CASCADE
    /*当更新Course 表中的cno 时，级联更新SC 表中相应的元组*/
    );

### 用户定义的完整性

not null、unique和check后面加条件
...
**记录**：事务标识、操作类型、操作对象、更新前数据的旧值、更新后数据的新值

**数据块**：事务标识、被更新的数据块

事务故障和系统故障恢复必须用日志文件，动态转储必须用、静态可以用

**登记**：登记次序按照事务发生顺序，先写日志后写数据库
...
二级：写数据加x，读数据加s锁，读完数据解s锁

三级：写数据加x，读数据加s锁，事务结束解s锁
