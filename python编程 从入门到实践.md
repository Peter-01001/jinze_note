...
给形参一个默认值，定义函数部分的形参列表中，必须右边的形参有默认值左边的才能有，即从右向左给默认值

### 传递列表

#### 函数中修改列表

函数中修改列表的值是永久的，即函数外函数也被改编

#### 禁止函数修改列表

传参时可以把列表的副本传给函数，这样函数修改的就是列表的副本
...
形参self必不可少，是一个指向实例本身的引用，使实例能够访问类内的属性和方法

### 继承

一个类继承另一个类时，子类继承父类所有属性和方法
