...
按照视点的不同将数据结构分为逻辑结构（外部结构）和物理结构/存储结构（内部结构）。
...
**线性表**（list）：由n（n>=0）个数据类型相同的数据元素a1，a2………，an组成的有限序列。

> n为表长，n=0时称为空表。
> 中间的数据元素有且仅有一个==直接前驱==和==直接后继==。

**位序**：ai是线性表中的一个数据元素，则i称为数据元素ai在线性表中的==位序==。

在较复杂的线性表中，一个数据元素可由多个数据项组成。
